from calculator_class2 import Calculator # import the class that handles the calculator functions
...
while ans != 'exit':
    ...
    try:       # try the below, and if it fails, move to the except blocks to catch errors.
        ...
        elif ans.isdecimal() or ans in operators or _isfloat(ans):
            ...
            if add_to_answer != 'yes' and add_to_answer != 'no':  # handling the case where a user inputs the wrong word
                while add_to_answer != 'yes' and add_to_answer != 'no':
                    ...
                    if add_to_answer == 'yes':
                        c.clear()
                        if result.split()[-1].isnumeric():
                            c.push(int(result.split()[-1]))
                
                        elif _isfloat(result.split()[-1]):
                            c.push(float(result.split()[-1]))
